              ...
              <div key={zoneLetter} className="flex justify-center gap-6 mb-4">
                {zones
                  ...
                  .map((zone) => {
                    ...
                    if (zone.price === 3000) bgColor = "bg-green-500"
                    else if (zone.price === 1500) bgColor = "bg-orange-500"
                    else if (zone.price === 1000) bgColor = "bg-sky-500"

                    return (
                      <div
                        key={zone.zoneName}
                        onClick={() => handleSectionSelect(zone.zoneName)}
                        className={`cursor-pointer w-20 h-16 rounded-lg ${bgColor} text-white font-bold flex items-center justify-center`}
                      >
                        {zone.zoneName}
                      </div>
                    )
